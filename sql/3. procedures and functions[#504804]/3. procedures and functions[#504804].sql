...
select * from products;
--------------------------------------------------------
create or replace procedure p_delete_data(u_id integer)
language 'plpgsql'
as $$
    BEGIN
...
--drop procedure p_delete_data(u_id integer);
...
drop function f_delete_data(i_name varchar);

call p_delete_data(3);

select f_delete_data('product_2');
